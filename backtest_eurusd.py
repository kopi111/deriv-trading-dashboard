"""
EUR/USD Backtesting Engine
Adapted from AUD/USD engine for EUR/USD forex pair.
...
def fetch_eurusd_data(period="5y", interval="1d"):
    """Fetch EUR/USD historical data via yfinance."""
...
def add_indicators(df):
    ...
    # Pips moved (for EUR/USD, 1 pip = 0.0001)
...
def run_all_strategies(period="5y"):
    """Run all strategies on EUR/USD and return results."""
    df = fetch_eurusd_data(period=period)
...
if __name__ == "__main__":
    ...
    print(f"\nEUR/USD Backtest Results (5y daily data, {len(df)} bars)")
    print(f"{'='*85}")
    ...
    # Show last 10 trades from best strategy
    print(f"\nLast 10 trades ({best[0]}):")
    print(f"{'Date':<12} {'Type':<5} {'Entry':>8} {'PnL':>8} {'Result':<5}")
    for t in best[1]["trades"][-10:]:
